...
> 
> **제작: [트위터: 꾸구 (@iforggu)](https://twitter.com/iforggu)**
> 
...
3-1. (윈도우) 압축 해제한 폴더로 이동한 뒤, 아래 사진처럼 파일 탐색기 창의 주소줄을 모두 지우고 `cmd`를 입력합니다.
![압축 해제한 폴더로 이동한 뒤, 아래 사진처럼 파일 탐색기 창의 주소줄을 모두 지우고 'cmd'를 입력합니다.](./misc/3.png)
   
3-2. (맥) 압축 해제한 폴더를 우클릭한 뒤, `폴더에서 새로운 터미널 열기`를 클릭합니다. (참고: [블로그](https://herojoon-dev.tistory.com/165))
   
4. cmd 또는 터미널에서 아래 명령어를 실행합니다.
```bash
npm install
```
...
설치나 개조 중 궁금한 사항, 기능 추가 요청, 버그 제보 등 각종 문의는 [GitHub의 Issues 탭](https://github.com/bg3-sheet/adventure/issues)이나 [트위터 계정](https://twitter.com/iforggu) 멘션, DM에 보내 주세요.
